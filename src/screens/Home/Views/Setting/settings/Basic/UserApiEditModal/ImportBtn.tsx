...
export default ({ btnStyle }: BtnProps) => {
  ...
  const mergedBtnStyle = useMemo(() => {
    if (!IS_TV_APP) return btnStyle
    return [
      {
  ...
  return (
    <DorpDownMenu
      btnStyle={mergedBtnStyle}
      fontSize={IS_TV_APP ? 16 : 15}
      height={IS_TV_APP ? scaleSizeH(52) : undefined}
      menus={importTypes}
      center
      onPress={handleAction}
    >
      <Text size={IS_TV_APP ? 15 : 14} color={theme['c-button-font']}>{t('user_api_btn_import')}</Text>
