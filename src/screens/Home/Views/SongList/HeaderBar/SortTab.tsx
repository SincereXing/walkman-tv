...
export default forwardRef<SortTabType, SortTabProps>(({ onSortChange }, ref) => {
  ...
  return (
    <ScrollView ref={scrollViewRef} style={styles.container} keyboardShouldPersistTaps={'always'} horizontal {...tvScrollParentProps}>
      {
        sorts.map(s => (
          <TvPressable style={styles.button} onPress={() => { handleSortChange(s.id) }} key={s.id}>
