import { playList, pause, togglePlay } from '@/core/player/player'
...
const List = forwardRef<ListType, ListProps>(({ onShowMenu, onMuiltSelectMode, onSelectAll }, ref) => {
  ...
  const activeListId = useActiveListId()
  const playMusicInfo = usePlayMusicInfo()
  const isPlayState = useIsPlay()

  const handleTvPlayControl = (item: LX.Music.MusicInfo, index: number) => {
    const cur = playMusicInfo.musicInfo
    const sameList = playMusicInfo.listId === activeListId
    const isCurrent = Boolean(sameList && cur?.id === item.id)
    if (isCurrent && isPlayState) void pause()
    else if (isCurrent && !isPlayState) togglePlay()
    else void playList(activeListId, index)
  }
  ...
  const renderItem: FlatListType['renderItem'] = ({ item, index }) => {
    const cur = playMusicInfo.musicInfo
    const isCurrentRow = playMusicInfo.listId === activeListId && cur?.id === item.id
    const tvPlayShowsPause = Boolean(IS_TV_APP && isCurrentRow && isPlayState)
    return (
      <ListItem
        item={item}
        index={index}
  ...
  return (
    <FlatList
      ...
      keyExtractor={getkey}
      extraData={{ activeIndex, tv: isPlayState }}
