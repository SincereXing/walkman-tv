...
import { View, FlatList, useWindowDimensions, type FlatListProps, type NativeSyntheticEvent, type NativeScrollEvent, type LayoutChangeEvent } from 'react-native'
...
import { TV_HTML_LYRIC_ACTIVE } from '@/config/tvStyles'
...
const LrcLine = memo(({ line, lineNum, activeLine, onLayout }: LineProps) => {
  ...
  const textAlignSetting = useSettingValue('playDetail.style.align')
  /** 效果图歌词区为左侧纵向列表，左对齐 */
  const textAlign = IS_TV_APP ? 'left' : textAlignSetting
  const baseSize = (lrcFontSize / 10) * (IS_TV_APP ? 0.9 : 1)
  const active = activeLine == lineNum
  /** HTML 稿：当前行更大更亮 #4ade80，其余 #666 */
  const size = IS_TV_APP && active ? baseSize * (20 / 16) : baseSize
  const lineHeight = setSpText(size) * (IS_TV_APP ? 1.24 : 1.3)

  const colors = useMemo(() => {
    const activeLineMatch = activeLine == lineNum
    if (IS_TV_APP) {
      return activeLineMatch ? [
        TV_HTML_LYRIC_ACTIVE,
        '#888888',
        1,
      ] as const : [
        '#666666',
  ...
  const tvTextShadow = IS_TV_APP && active
    ? {
...
export default () => {
  ...
  /** TV：RN 中 paddingTop 百分比起算自宽度，原 '100%' 会把列表头垫到极高空，歌词滚出屏外 */
  const tvSpaceHeight = useMemo(
    () => Math.min(260, Math.max(96, Math.round(winH * 0.16))),
    [winH],
  )

  const spaceComponent = useMemo(() => (
    IS_TV_APP
      ? <View style={{ height: tvSpaceHeight }} onLayout={handleSpaceLayout} />
      : <View style={styles.space} onLayout={handleSpaceLayout} />
  ), [handleSpaceLayout, tvSpaceHeight])
  ...
  return (
    <>
      <FlatList
        ...
        keyExtractor={getkey}
        style={[styles.container, IS_TV_APP ? styles.containerTv : null]}
        ref={flatListRef}
