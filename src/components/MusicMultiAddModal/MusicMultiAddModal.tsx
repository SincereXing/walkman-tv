...
import CreateUserListDialog from '../MusicAddModal/CreateUserListDialog'
...
export default forwardRef<MusicMultiAddModalType, MusicMultiAddModalProps>(({ onAdded }, ref) => {
  ...
  return (
    <>
      <Dialog ref={dialogRef} onHide={handleHide} title={t(selectInfo.isMove ? 'move_to' : 'add_to')}>
        {
          selectInfo.selectedList.length
            ? (<>
                <Title selectedList={selectInfo.selectedList} isMove={selectInfo.isMove} />
                <List listId={selectInfo.listId} onPress={handleSelect} onCreate={handleShowCreate} />
              </>)
            : null
        }
      </Dialog>
      <CreateUserListDialog visible={createVisible} onHide={handleHideCreate} />
    </>
  )
})
