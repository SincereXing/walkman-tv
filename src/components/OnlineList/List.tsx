...
import { pause, togglePlay } from '@/core/player/player'
...
const List = forwardRef<ListType, ListProps>(({
  ...
  const handleTvPlayControl = (item: LX.Music.MusicInfoOnline, _index: number) => {
    const cur = playMusicInfo.musicInfo
    const onDefaultList = playMusicInfo.listId === LIST_IDS.DEFAULT
    const isCurrent = onDefaultList && cur?.id === item.id
    if (isCurrent && isPlay) {
      void pause()
    } else if (isCurrent && !isPlay) {
      togglePlay()
    } else {
      handlePlay(item)
    }
  ...
  const handlePress = (item: LX.Music.MusicInfoOnline, index: number) => {
    requestAnimationFrame(() => {
      ...
      if (isMultiSelectModeRef.current) {
        ...
        const cid = commonState.componentIds.home
        if (cid) requestAnimationFrame(() => { navigations.pushPlayDetailScreen(cid) })
      } else if (settingState.setting['list.isClickPlayList'] && onPlayList != null) {
        onPlayList(index)
      } else {
        handlePlay(currentList[index])
      }
  ...
  const renderItem: FlatListType['renderItem'] = ({ item, index }) => {
    const cur = playMusicInfo.musicInfo
    const isCurrentRow = playMusicInfo.listId === LIST_IDS.DEFAULT && cur?.id === item.id
    const tvPlayShowsPause = Boolean(IS_TV_APP && isCurrentRow && isPlay)
    return (
      <ListItem
        item={item}
        index={index}
        ...
        tvPlayShowsPause={tvPlayShowsPause}
        onTvPlayControl={IS_TV_APP ? handleTvPlayControl : undefined}
      />
    )
  }
  ...
  return (
    <FlatList
      ref={flatListRef}
      style={styles.list}
      contentContainerStyle={IS_TV_APP ? styles.listContent : undefined}
      data={currentList}
      ...
      windowSize={8}
      removeClippedSubviews={!IS_TV_APP}
      initialNumToRender={12}
      renderItem={renderItem}
      keyExtractor={getkey}
      getItemLayout={getItemLayout}
      extraData={{ isPlay, playingId: playMusicInfo.musicInfo?.id, listId: playMusicInfo.listId }}
      // onRefresh={onRefresh}
          ...
          <View>
            {
              label === 'list_error'
                ? (
                    <TvPressable style={styles.footerPressable} onPress={handlePress}>
                      <Text style={styles.footerText} color={theme['c-font-label']}>{t(label)}</Text>
                    </TvPressable>
                  )
                : (
                    <Text style={styles.footer} color={theme['c-font-label']}>{t(label)}</Text>
                  )
            }
          </View>
        )
