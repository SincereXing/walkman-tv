...
export default ({ check, label, children, onChange, helpTitle, helpDesc, disabled = false, need = false, marginRight = 0, marginBottom = 0, size = 1 }: CheckBoxProps) => {
  ...
  const helpComponent = useMemo(() => {
    ...
    return (helpTitle ?? helpDesc) ? (
      <TvPressable style={styles.helpBtn} onPress={handleShowHelp}>
    ...
    disabled
      ? (
          <View style={contentStyle}>
            ...
            <View style={labelStyle}>{label ? <Text style={styles.name} color={IS_TV_APP ? 'rgba(255,255,255,0.5)' : theme['c-500']} size={15 * size}>{label}</Text> : children}</View>
            {helpComponent}
          </View>
        )
      : (
          <View style={contentStyle}>
            ...
            <TvPressable
              style={IS_TV_APP ? { ...labelStyle, paddingVertical: 10, minHeight: scaleSizeH(44), justifyContent: 'center' } : labelStyle}
