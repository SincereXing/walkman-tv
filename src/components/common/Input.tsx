...
export default forwardRef<InputType, InputProps>(({ onChangeText, onClearText, clearBtn, style, size = 14, onFocus, onBlur, ...props }, ref) => {
  const inputRef = useRef<TextInput>(null)
  const theme = useTheme()
  const [tvInputFocused, setTvInputFocused] = useState(false)
  ...
  const handleFocus = useCallback<NonNullable<TextInputProps['onFocus']>>((e) => {
    if (IS_TV_APP) setTvInputFocused(true)
    onFocus?.(e)
  }, [onFocus])

  const handleBlur = useCallback<NonNullable<TextInputProps['onBlur']>>((e) => {
    if (IS_TV_APP) setTvInputFocused(false)
    onBlur?.(e)
  }, [onBlur])

  const tvOutline = IS_TV_APP && tvInputFocused
    ? { borderWidth: TV_FOCUS_BORDER_WIDTH, borderColor: TV_FOCUS_BORDER, borderRadius: 6, padding: 1 }
    : undefined

  return (
    <View style={StyleSheet.compose(styles.content, tvOutline)}>
      <TextInput
        autoCapitalize="none"
      ...
      <Animated.View style={{ ...styles.clearBtnContent, transform: [{ scale: scaleClearBtn }] }}> */}
        {clearBtn
          ? <View style={styles.clearBtnContent}>
              <TvPressable
                style={IS_TV_APP ? { ...styles.clearBtn, minWidth: 40, minHeight: 40, justifyContent: 'center', alignItems: 'center' } : styles.clearBtn}
                onPress={clearText}
              >
