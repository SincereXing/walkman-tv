...
const pickMvPic = (info) => {
  if (info.hts_MVPIC) return info.hts_MVPIC
  if (info.MVPIC) return `https://img1.kuwo.cn/wmvpic/${info.MVPIC}`
  return null
}
  ...
  handleResult(rawData) {
    ...
    for (let i = 0; i < rawData.length; i++) {
      ...
      result.push({
        ...
        mvId: info.mvpayinfo?.vid && info.mvpayinfo.vid !== '0' ? info.mvpayinfo.vid : undefined,
        mvPlayable: info.mvpayinfo?.play === '1',
        mvPic: pickMvPic(info),
        source: 'kw',
