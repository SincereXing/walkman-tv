...
const getMvHash = (item) => {
  const list = [item.mvhash, item.MvHash, item.mv_hash, item.MVHash, item.base?.mv_hash, item.base?.mvhash]
  const value = list.find(v => v != null && v !== '')
  ...
  filterData(rawList) {
    ...
    return rawList.map(item => {
      ...
      return {
        ...
        lrc: null,
        hash: item.hash,
        mvHash: getMvHash(item) || undefined,
        types,
        _types,
        typeUrl: {},
      }
