...
const getMvMeta = (item) => {
  const mv = item.mv ?? {}
  const mvVid = String(mv.vid ?? mv.mv_vid ?? '').trim()
...
export const getMusicInfoRaw = async(songmid) => {
  ...
  return requestObj.promise.then(({ body }) => {
    ...
    return body.req.data.track_info
  })
}

export default (songmid) => {
  return getMusicInfoRaw(songmid).then(item => {
    ...
    const { mvId, mvVid } = getMvMeta(item)
    return {
