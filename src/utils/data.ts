...
import { BUILTIN_USER_API_SCRIPT } from '@/core/builtinUserApi/builtinUserApiScript.generated'
...
const normalizeSongListSetting = (setting: typeof DEFAULT_SETTING['songList']) => {
  if (setting.source === 'kw' && setting.sortId === 'new' && !setting.tagId && !setting.tagName) {
    return { ...DEFAULT_SETTING.songList }
  }
  return setting
}

const normalizeLeaderboardSetting = (setting: typeof DEFAULT_SETTING['leaderboard']) => {
  if (setting.source === 'kw' && setting.boardId === 'kw__16') {
    return { ...DEFAULT_SETTING.leaderboard }
  }
  return setting
}
...
export const getSongListSetting = async() => {
  // eslint-disable-next-line require-atomic-updates
  songListSetting ??= normalizeSongListSetting(await getData(songListSettingKey) ?? { ...DEFAULT_SETTING.songList })
...
export const getLeaderboardSetting = async() => {
  // eslint-disable-next-line require-atomic-updates
  leaderboardSetting ??= normalizeLeaderboardSetting(await getData(leaderboardSettingKey) ?? { ...DEFAULT_SETTING.leaderboard })
...
/** 首次启动时注入内置 v4.0 脚本；已存在同 ID 则跳过 */
export const ensureBuiltinUserApi = async(): Promise<void> => {
  const list = await getUserApiList()
  if (list.some(a => a.id === BUILTIN_USER_API_ID)) return

  const script = BUILTIN_USER_API_SCRIPT
  const result = /^\/\*[\S|\s]+?\*\//.exec(script)
  if (!result) {
    console.warn('[lx-music-tv] builtin user API script: header parse failed')
    return
  }
  const scriptInfo = matchInfo(result[0])
  scriptInfo.name ||= '[内置音源]'
  const apiInfo: LX.UserApi.UserApiInfo = {
  ...
  await saveDataMultiple([
    [userApiPrefix, userApis],
    [`${userApiPrefix}${BUILTIN_USER_API_ID}`, script],
  ])
}
